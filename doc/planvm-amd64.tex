...
the items marked in the explore_tree.
...
But we need a second area to mark freshly malloced items that were generated
while the exploration and sweeping were in progress. We don't want to overload
the sweep_tree_ptr because you then confuse marking new items with whether
you've recursively iterated through them, leading to undermarking.
...
Given a 64-bit word of child mark bits (packed as [child0, child1, child2,
... child63], where the children for parent i are at positions \lstinline|2*i|
and \lstinline|2*i+1|), compute for each parent: parent's bit =
\lstinline|child_left| OR \lstinline|child_right|.
